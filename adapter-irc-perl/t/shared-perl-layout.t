...
File::Find::find(
  ...
  map { File::Spec->catdir($root, $_) } qw(lib t xt)
);

for my $path (sort @perl_files) {
  next if $path =~ /shared-perl-layout\.t\z/mx;
  ...
  unlike $source, qr/local.*lib.*perl5/smx, "$path does not assemble local-lib include paths";
}
